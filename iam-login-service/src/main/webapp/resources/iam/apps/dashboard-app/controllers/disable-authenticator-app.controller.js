 ...
 */
  ...
  function DisableAuthenticatorAppController(
      $scope, $state, $uibModalInstance, Utils, user) {
    var disableAuthAppCtrl = this;

    disableAuthAppCtrl.userToEdit = user;
    disableAuthAppCtrl.codeMinlength = 6;

    disableAuthAppCtrl.dismiss = dismiss;
    disableAuthAppCtrl.reset = reset;

    function reset() {
      console.log('reset form');

      disableAuthAppCtrl.disabled = true;

      disableAuthAppCtrl.user = {
        code: ''
      };
    ...
    disableAuthAppCtrl.reset();

    function dismiss() { return $uibModalInstance.dismiss('Cancel'); }

    disableAuthAppCtrl.message = '';

    disableAuthAppCtrl.submit = function() {
