CREATE TABLE iam_account (ID BIGINT AUTO_INCREMENT NOT NULL, active TINYINT(1) default 0 NOT NULL, CREATIONTIME DATETIME NOT NULL, LASTUPDATETIME DATETIME NOT NULL, PASSWORD VARCHAR(128), USERNAME VARCHAR(128) NOT NULL UNIQUE, UUID VARCHAR(36) NOT NULL UNIQUE, user_info_id BIGINT, mfa_enabled TINYINT(1) default 0 NOT NULL, totp_mfa_id BIGINT, PRIMARY KEY (ID));
...
CREATE TABLE iam_totp_mfa (ID BIGINT NOT NULL, active TINYINT(1) default 0 NOT NULL, CREATIONTIME DATETIME NOT NULL, LASTUPDATETIME DATETIME NOT NULL, secret VARCHAR(256), ACCOUNT_ID BIGINT, PRIMARY KEY (ID));
CREATE TABLE iam_totp_recovery_code (ID BIGINT NOT NULL, code VARCHAR(255) NOT NULL, CREATIONTIME DATETIME NOT NULL, LASTUPDATETIME DATETIME NOT NULL, TOTP_MFA_ID BIGINT NOT NULL, PRIMARY KEY (ID));
